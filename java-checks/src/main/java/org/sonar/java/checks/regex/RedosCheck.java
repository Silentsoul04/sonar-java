...
import org.sonar.java.checks.helpers.RegexTreeHelper.CachedIntersectionChecker;
import org.sonar.java.checks.helpers.RegexTreeHelper.CachedReachabilityChecker;
...
public class RedosCheck extends AbstractRegexCheckTrackingMatchType {
  ...
  private final CachedReachabilityChecker reachabilityChecker = new CachedReachabilityChecker(false);
  private final CachedIntersectionChecker intersectionChecker = new CachedIntersectionChecker(false);
      ...
      if (tree.getQuantifier().isOpenEnded() && canFail(tree)) {
        for (RepetitionTree repetition : repetitions) {
          if (reachabilityChecker.canReach(repetition, tree)) {
            ...
            if (intersectionChecker.intersects(auto1, auto2)) {
