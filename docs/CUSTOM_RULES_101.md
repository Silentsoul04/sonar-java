...
Grab the template project by cloning this repository (https://github.com/SonarSource/sonar-java) and then importing in your IDE the sub-module: https://github.com/SonarSource/sonar-java/tree/master/docs/java-custom-rules-example
This project already contains examples of custom rules. Our goal will be to add an extra rule!

### Looking at the POM

A custom plugin is a Maven project, and before diving into code, it is important to notice a few relevant lines related to the configuration of your soon-to-be-released custom plugin. The root of a Maven project is a file named `pom.xml`. In our case, we have two of them: `pom.xml` and `pom_self_contained.xml`. **We will only use in this tutorial the file named `pom_self_contained.xml`**, as it is completely independent from the build of the Java Analyzer, and is self contained.
...
You can also decide to **delete** the original pom.xml file, and then rename `pom_self_contained.xml` into `pom.xml`. You would then be able to use the slightly simpler following command:

```
mvn clean install -Dsonarqube.version=8.7.0.41497 -Dsonarjava.version=6.13.0.25138
```

These 2 commands forces usages of a given version of SonarQube (should correspond to your instance version), as well as the version of the java analyzer to be consider. The Java analyzer version should match the one embedded in your SonarQube instance. Note that **we strongly recommend you to always use the latest version of SonarQube**.

The property `sonarqube.version` relates to the minimum version of SonarQube your custom-rules plugin will support, and is generally aligned to your company's SonarQube instance. In this template, we rely on the version **8.7.0.41497** (the LTS version is **7.9.x**, but compatibility to latest release is guaranteed when packaging the plugin).

The property `<sonarjava.version>` is the minimum version of the Java Analyzer that will be required to run your custom plugin in your SonarQube instance. Consequently, as we will rely on version **6.13.0.25138** of the Java Analyzer, the SonarQube instance which will use the custom plugin will need version **6.13.0.25138** of the Java Analyzer to be installed as well.

By default, you will be forced to use this full command to build the plugin, but once you know your SonarQube and Java Analyzer version, you can decide to uncomment the corresponding parts in the pom:

```xml
<properties>
  <!-- Uncomment and replace this versions by your SonarQube version -->
  <!-- <sonarqube.version>8.7.0.41497</sonarqube.version> -->
  <!-- Uncomment and replace this version by the version of the SonarJava plugin embedded in your SonarQube version-->
...
It's also the property `<sonarQubeMinVersion>` which guarantees the compatibility with SonarQube LTS 7.9.
...
<plugin>
  ...
  <configuration>
    ...
    <!-- allows to depend on API 8.x but still run on LTS 7.9 -->
...
Start by building the project using maven. Note that here we are using the self-contained `pom` file. If you renamed it into `pom.xml`, remove the `-f pom_self_contained.xml` part of the following command):
...
See: [pom.xml#L136-L196](https://github.com/SonarSource/sonar-java/blob/custom_rules_example/docs/java-custom-rules-example/pom_self_contained.xml#L136-L196)
